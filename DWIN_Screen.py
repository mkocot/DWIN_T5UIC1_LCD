...
class T5UIC1_LCD:
    ...
    def Handshake(self):
        ...
        while (self.MYSERIAL1.in_waiting and self.recnum < 26):
            ...
            self.databuf[self.recnum] = struct.unpack(
                'B', self.MYSERIAL1.read()
            )[0]
    ...
    def Draw_Circle(self, Color, x0, y0, r):  # Draw a circle
        ...
        while(a <= b):
            ...
            while(a == 0):
                ...
                break
            # Draw some sector 1
            self.DrawPoint(Color, 1, 1, x0 + a, y0 + b)
            # Draw some sector 2
            self.DrawPoint(Color, 1, 1, x0 + b, y0 + a)
            # Draw some sector 3
            self.DrawPoint(Color, 1, 1, x0 + b, y0 - a)
            # Draw some sector 4
            self.DrawPoint(Color, 1, 1, x0 + a, y0 - b)
        ...
        for i in range(r, 0, -1):
            ...
            while(a <= b):
                ...
                while(a == 0):
                    ...
                    break
                # Draw some sector 1
                self.DrawPoint(FColor, 2, 2, x0 + a, y0 + b)
                # raw some sector 2
                self.DrawPoint(FColor, 2, 2, x0 + b, y0 + a)
                # Draw some sector 3
                self.DrawPoint(FColor, 2, 2, x0 + b, y0 - a)
                # Draw some sector 4
                self.DrawPoint(FColor, 2, 2, x0 + a, y0 - b)
    ...
    def Draw_IntValue(self, bShow, zeroFill, zeroMode, size, color, bColor, iNum, x, y, value):
        ...
        # Bit 4: zeroMode
        # Bit 3-0: size
        self.Byte((bShow * 0x80) | (zeroFill * 0x20)
                  | (zeroMode * 0x10) | size)
        self.Word(color)
        self.Word(bColor)
        self.Byte(iNum)
    ...
    def QR_Code(self, QR_Pixel, Xs, Ys, data):	    # Display QR code
        ...
        if(QR_Pixel <= 6):  # Set the upper limit of pixels according to the actual screen size
            ...
            # The pixel size of the QR code exceeds the default of 1
            self.Byte(0x06)
        self.String(data)
        self.Send()
    # /*---------------------------------------- Memory functions ----------------------------------------*/
