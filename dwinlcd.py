...
class DWIN_LCD:
    ...
    def __init__(self, USARTx, encoder_pins, button_pin, octoPrint_API_Key):
        ...
        GPIO.add_event_detect(self.button_pin, GPIO.BOTH,
                              callback=self.encoder_has_data)
        ...
        self.timer = multitimer.MultiTimer(
    ...
    def HMI_ShowBoot(self, mesg=None):
        ...
        for t in range(0, 100, 2):
            ...
            self.lcd.Draw_Rectangle(
                1, self.lcd.Color_Bg_Black, 15 + t * 242 / 100, 260, 257, 280)
            self.lcd.UpdateLCD()
            time.sleep(.020)
        ...
        if (encoder_diffState == self.ENCODER_DIFF_CW and fullCnt):
            ...
            if (self.select_file.dec()):
                ...
                if (self.select_file.now < self.index_file - self.MROWS):  # Cursor past the top
                    ...
                    # Just move highlight
                    self.Move_Highlight(-1, self.select_file.now +
                                        self.MROWS - self.index_file)
        elif (encoder_diffState == self.ENCODER_DIFF_ENTER):
        ...
        if (encoder_diffState == self.ENCODER_DIFF_CW):
            ...
            if (self.select_prepare.dec()):
                if (self.select_prepare.now < self.index_prepare - self.MROWS):
                    ...
                    self.Move_Highlight(-1, self.select_prepare.now +
            ...
            if (self.select_prepare.now == 0):  # Back
                ...
                self.lcd.Draw_FloatValue(
                    ...
                    3, 1, 216, self.MBASE(
                        1), self.pd.current_position.x * self.MINUNITMULT
                )
                self.lcd.Draw_FloatValue(
                    True, True, 0, self.lcd.font8x16, self.lcd.Color_White, self.lcd.Color_Bg_Black,
                    3, 1, 216, self.MBASE(
                        2), self.pd.current_position.y * self.MINUNITMULT
                )
                self.lcd.Draw_FloatValue(
                    True, True, 0, self.lcd.font8x16, self.lcd.Color_White, self.lcd.Color_Bg_Black,
                    3, 1, 216, self.MBASE(
                        3), self.pd.current_position.z * self.MINUNITMULT
                )
        ...
        if (encoder_diffState == self.ENCODER_DIFF_CW):
            if (self.select_control.inc(1 + self.CONTROL_CASE_TOTAL)):
                if (self.select_control.now > self.MROWS and self.select_control.now > self.index_control):
                    ...
                    self.Draw_More_Icon(
                        self.CONTROL_CASE_TEMP + self.MROWS - self.index_control)  # Temperature >
                    self.Draw_More_Icon(
                        self.CONTROL_CASE_MOVE + self.MROWS - self.index_control)  # Motion >
                    if (self.index_control > self.MROWS):
                        self.Draw_More_Icon(
                            self.CONTROL_CASE_INFO + self.MROWS - self.index_control)  # Info >
                        self.lcd.Frame_AreaCopy(
                            1, 0, 104, 24, 114, self.LBLX, self.MBASE(self.CONTROL_CASE_INFO - 1))
                else:
                    self.Move_Highlight(
            ...
            if (self.select_control.dec()):
                if (self.select_control.now < self.index_control - self.MROWS):
                    ...
                    self.Draw_More_Icon(
                        0 + self.MROWS - self.index_control + 1)  # Temperature >
                    self.Draw_More_Icon(
                        1 + self.MROWS - self.index_control + 1)  # Motion >
                else:
                    self.Move_Highlight(-1, self.select_control.now +
                                        self.MROWS - self.index_control)
        elif (encoder_diffState == self.ENCODER_DIFF_ENTER):
        ...
        if (encoder_diffState == self.ENCODER_DIFF_CW):
            ...
            if (self.select_tune.dec()):
                if (self.select_tune.now < self.index_tune - self.MROWS):
                    ...
                    self.Move_Highlight(-1, self.select_tune.now +
                                        self.MROWS - self.index_tune)
        elif (encoder_diffState == self.ENCODER_DIFF_ENTER):
    ...
    def HMI_PrintSpeed(self):
        ...
        self.lcd.Draw_IntValue(
            True, True, 0, self.lcd.font8x16, self.lcd.Color_White, self.lcd.Color_Bg_Black,
            3, 216, self.MBASE(self.select_tune.now +
                               self.MROWS - self.index_tune),
    ...
    def HMI_AxisMove(self):
        ...
        if self.pd.PREVENT_COLD_EXTRUSION:
            ...
            if (self.pd.HMI_flag.ETempTooLow_flag):
                if (encoder_diffState == self.ENCODER_DIFF_ENTER):
                    ...
                    self.lcd.Draw_Signed_Float(
                        self.lcd.font8x16, self.lcd.Color_Bg_Black, 3, 1, 216, self.MBASE(
                            4), 0
                    )
                    self.lcd.UpdateLCD()
                return
        ...
        if (encoder_diffState == self.ENCODER_DIFF_CW):
            ...
            if self.select_axis.now == 0:  # Back
                ...
                self.lcd.Draw_Signed_Float(
                    self.lcd.font8x16, self.lcd.Select_Color, 3, 1, 216, self.MBASE(
                        4),
    ...
    def HMI_Move_E(self):
        ...
        if ((self.pd.HMI_ValueStruct.Move_E_scale - self.pd.last_E_scale) > (self.pd.EXTRUDE_MAXLENGTH) * self.MINUNITMULT):
            self.pd.HMI_ValueStruct.Move_E_scale = self.pd.last_E_scale + \
        ...
        self.lcd.Draw_Signed_Float(self.lcd.font8x16, self.lcd.Select_Color, 3, 1, 216, self.MBASE(
            4), self.pd.HMI_ValueStruct.Move_E_scale)
        self.lcd.UpdateLCD()
        ...
        if (encoder_diffState == self.ENCODER_DIFF_CW):
            ...
            if self.select_temp.now == 0:  # back
                ...
                self.lcd.Draw_IntValue(
                    True, True, 0, self.lcd.font8x16, self.lcd.Color_White, self.lcd.Select_Color,
                    3, 216, self.MBASE(
                        3), self.pd.thermalManager['fan_speed'][0]
                )
                self.EncoderRateLimit = False
                ...
                self.lcd.Frame_AreaCopy(
                    1, 157, 76, 181, 86, self.LBLX, self.MBASE(self.PREHEAT_CASE_TEMP))
                self.lcd.Frame_AreaCopy(
                    1, 197, 104, 238, 114, self.LBLX + 27, self.MBASE(self.PREHEAT_CASE_TEMP))
                self.lcd.Frame_AreaCopy(
                    1, 1, 89, 83, 101, self.LBLX + 71, self.MBASE(self.PREHEAT_CASE_TEMP))  # PLA nozzle temp
                if self.pd.HAS_HEATED_BED:
                    self.lcd.Frame_AreaCopy(
                        1, 157, 76, 181, 86, self.LBLX, self.MBASE(self.PREHEAT_CASE_BED) + 3)
                    self.lcd.Frame_AreaCopy(
                        1, 240, 104, 264, 114, self.LBLX + 27, self.MBASE(self.PREHEAT_CASE_BED) + 3)
                    self.lcd.Frame_AreaCopy(
                        1, 1, 89, 83, 101, self.LBLX + 54, self.MBASE(self.PREHEAT_CASE_BED) + 3)  # PLA bed temp
                if self.pd.HAS_FAN:
                    self.lcd.Frame_AreaCopy(
                        1, 157, 76, 181, 86, self.LBLX, self.MBASE(self.PREHEAT_CASE_FAN))
                    self.lcd.Frame_AreaCopy(
                        1, 0, 119, 64, 132, self.LBLX + 27, self.MBASE(self.PREHEAT_CASE_FAN))  # PLA fan speed

                self.lcd.Frame_AreaCopy(1, 97, 165, 229, 177, self.LBLX, self.MBASE(
                    self.PREHEAT_CASE_SAVE))  # Save PLA configuration
                ...
                self.lcd.Frame_AreaCopy(
                    1, 172, 76, 198, 86, self.LBLX, self.MBASE(self.PREHEAT_CASE_TEMP))
                self.lcd.Frame_AreaCopy(
                    1, 197, 104, 238, 114, self.LBLX + 27, self.MBASE(self.PREHEAT_CASE_TEMP))
                self.lcd.Frame_AreaCopy(
                    1, 1, 89, 83, 101, self.LBLX + 71, self.MBASE(self.PREHEAT_CASE_TEMP))  # ABS nozzle temp
                if self.pd.HAS_HEATED_BED:
                    self.lcd.Frame_AreaCopy(
                        1, 172, 76, 198, 86, self.LBLX, self.MBASE(self.PREHEAT_CASE_BED) + 3)
                    self.lcd.Frame_AreaCopy(
                        1, 240, 104, 264, 114, self.LBLX + 27, self.MBASE(self.PREHEAT_CASE_BED) + 3)
                    self.lcd.Frame_AreaCopy(
                        1, 1, 89, 83, 101, self.LBLX + 54, self.MBASE(self.PREHEAT_CASE_BED) + 3)  # ABS bed temp
                if self.pd.HAS_FAN:
                    self.lcd.Frame_AreaCopy(
                        1, 172, 76, 198, 86, self.LBLX, self.MBASE(self.PREHEAT_CASE_FAN))
                    self.lcd.Frame_AreaCopy(
                        1, 0, 119, 64, 132, self.LBLX + 27, self.MBASE(self.PREHEAT_CASE_FAN))  # ABS fan speed

                self.lcd.Frame_AreaCopy(
                    1, 97, 165, 229, 177, self.LBLX, self.MBASE(self.PREHEAT_CASE_SAVE))
                self.lcd.Frame_AreaCopy(1, 172, 76, 198, 86, self.LBLX + 33,
                                        self.MBASE(self.PREHEAT_CASE_SAVE))  # Save ABS configuration
...
# ---------------------Todo--------------------------------#
    ...
    def HMI_Zoffset(self):
        ...
        if (encoder_diffState == self.ENCODER_DIFF_ENTER):
            ...
            self.lcd.Draw_Signed_Float(
                self.lcd.font8x16, self.lcd.Color_Bg_Black, 2, 2, 202, self.MBASE(
                    zoff_line),
                self.pd.HMI_ValueStruct.offset_value
            )
    ...
    def Draw_Status_Area(self, with_update):
        ...
        self.lcd.Draw_Rectangle(1, self.lcd.Color_Bg_Black, 0,
                                self.STATUS_Y, self.lcd.DWIN_WIDTH, self.lcd.DWIN_HEIGHT - 1)
        #
        #  Status Area
        #
        if self.pd.HAS_HOTEND:
    ...
    def Draw_Title(self, title):
        self.lcd.Draw_String(False, False, self.lcd.DWIN_FONT_HEAD,
                             self.lcd.Color_White, self.lcd.Color_Bg_Blue, 14, 4, title)
    ...
    def Draw_More_Icon(self, line):
        self.lcd.ICON_Show(self.ICON, self.ICON_More,
                           226, self.MBASE(line) - 3)

    def Draw_Menu_Cursor(self, line):
        self.lcd.Draw_Rectangle(1, self.lcd.Rectangle_Color, 0, self.MBASE(
            line) - 18, 14, self.MBASE(line + 1) - 20)
    ...
    def Draw_Menu_Line(self, line, icon=False, label=False):
        if (label):
            self.lcd.Draw_String(False, False, self.lcd.font8x16, self.lcd.Color_White,
                                 self.lcd.Color_Bg_Black, self.LBLX, self.MBASE(line) - 1, label)
        if (icon):
            self.Draw_Menu_Icon(line, icon)
        self.lcd.Draw_Line(self.lcd.Line_Color, 16, self.MBASE(
            line) + 33, 256, self.MBASE(line) + 34)

    # The "Back" label is always on the first line
    def Draw_Back_Label(self):
        self.lcd.Frame_AreaCopy(1, 226, 179, 256, 189,
    ...
    def draw_max_en(self, line):
        self.lcd.Frame_AreaCopy(1, 245, 119, 269, 129,
                                self.LBLX, line)  # "Max"

    def draw_max_accel_en(self, line):
        self.draw_max_en(line)
        self.lcd.Frame_AreaCopy(
            1, 1, 135, 79, 145, self.LBLX + 27, line)  # "Acceleration"

    def draw_speed_en(self, inset, line):
        self.lcd.Frame_AreaCopy(1, 184, 119, 224, 132,
                                self.LBLX + inset, line)  # "Speed"

    def draw_jerk_en(self, line):
        self.lcd.Frame_AreaCopy(1, 64, 119, 106, 129,
    ...
    def Draw_Print_ProgressBar(self, Percentrecord=None):
        ...
        self.lcd.Draw_Rectangle(1, self.lcd.BarFill_Color,
                                16 + Percentrecord * 240 / 100, 93, 256, 113)
        self.lcd.Draw_IntValue(True, True, 0, self.lcd.font8x16, self.lcd.Percent_Color,
                               self.lcd.Color_Bg_Black, 2, 117, 133, Percentrecord)
    ...
    def Draw_Print_ProgressElapsed(self):
        ...
        self.lcd.Draw_IntValue(True, True, 1, self.lcd.font8x16, self.lcd.Color_White,
                               self.lcd.Color_Bg_Black, 2, 42, 212, elapsed / 3600)
        self.lcd.Draw_String(False, False, self.lcd.font8x16,
                             self.lcd.Color_White, self.lcd.Color_Bg_Black, 58, 212, ":")
        self.lcd.Draw_IntValue(True, True, 1, self.lcd.font8x16, self.lcd.Color_White,
                               self.lcd.Color_Bg_Black, 2, 66, 212, (elapsed % 3600) / 60)
    ...
    def Draw_Print_ProgressRemain(self):
        ...
        self.lcd.Draw_IntValue(True, True, 1, self.lcd.font8x16, self.lcd.Color_White,
                               self.lcd.Color_Bg_Black, 2, 176, 212, remain_time / 3600)
        self.lcd.Draw_String(False, False, self.lcd.font8x16,
                             self.lcd.Color_White, self.lcd.Color_Bg_Black, 192, 212, ":")
        self.lcd.Draw_IntValue(True, True, 1, self.lcd.font8x16, self.lcd.Color_White,
                               self.lcd.Color_Bg_Black, 2, 200, 212, (remain_time % 3600) / 60)
    ...
    def Draw_Prepare_Menu(self):
        ...
        if self.pd.HAS_ZOFFSET_ITEM:
            if scroll + self.PREPARE_CASE_ZOFF <= self.MROWS:
                ...
                self.Item_Prepare_Offset(self.PREPARE_CASE_ZOFF)
        if self.pd.HAS_HOTEND:
    ...
    def Draw_Control_Menu(self):
        ...
        self.lcd.Frame_AreaCopy(1, 1, 89, 83, 101, self.LBLX, self.MBASE(
            self.CONTROL_CASE_TEMP))  # Temperature >
        self.lcd.Frame_AreaCopy(1, 84, 89, 128, 99, self.LBLX, self.MBASE(
            self.CONTROL_CASE_MOVE))  # Motion >
        self.lcd.Frame_AreaCopy(1, 0, 104, 25, 115, self.LBLX, self.MBASE(
            self.CONTROL_CASE_INFO))  # Info >
    ...
    def Draw_Info_Menu(self):
        ...
        self.lcd.Draw_String(
            ...
            (self.lcd.DWIN_WIDTH - len(self.pd.MACHINE_SIZE)
             * self.MENU_CHR_W) / 2, 122,
            self.pd.MACHINE_SIZE
        )
        self.lcd.Draw_String(
            False, False, self.lcd.font8x16, self.lcd.Color_White, self.lcd.Color_Bg_Black,
            (self.lcd.DWIN_WIDTH - len(self.pd.SHORT_BUILD_VERSION)
             * self.MENU_CHR_W) / 2, 195,
        ...
        self.lcd.Draw_String(
            False, False, self.lcd.font8x16, self.lcd.Color_White, self.lcd.Color_Bg_Black,
            (self.lcd.DWIN_WIDTH - len(self.pd.CORP_WEBSITE_E)
             * self.MENU_CHR_W) / 2, 268,
            self.pd.CORP_WEBSITE_E
        )
        self.Draw_Back_First()
        for i in range(3):
            self.lcd.ICON_Show(
                self.ICON, self.ICON_PrintSize + i, 26, 99 + i * 73)
    ...
    def Draw_Tune_Menu(self):
        ...
        self.lcd.Frame_AreaCopy(1, 1, 179, 92, 190, self.LBLX, self.MBASE(
            self.TUNE_CASE_SPEED))  # Print speed
        if self.pd.HAS_HOTEND:
            # Hotend...
            self.lcd.Frame_AreaCopy(
                1, 197, 104, 238, 114, self.LBLX, self.MBASE(self.TUNE_CASE_TEMP))
            self.lcd.Frame_AreaCopy(
                1, 1, 89, 83, 101, self.LBLX + 44, self.MBASE(self.TUNE_CASE_TEMP))  # Temperature
        if self.pd.HAS_HEATED_BED:
            # Bed...
            self.lcd.Frame_AreaCopy(
                1, 240, 104, 264, 114, self.LBLX, self.MBASE(self.TUNE_CASE_BED))
            self.lcd.Frame_AreaCopy(
                1, 1, 89, 83, 101, self.LBLX + 27, self.MBASE(self.TUNE_CASE_BED))  # ...Temperature
        if self.pd.HAS_FAN:
            self.lcd.Frame_AreaCopy(1, 0, 119, 64, 132, self.LBLX, self.MBASE(
                self.TUNE_CASE_FAN))  # Fan speed
        if self.pd.HAS_ZOFFSET_ITEM:
            self.lcd.Frame_AreaCopy(1, 93, 179, 141, 189, self.LBLX, self.MBASE(
                self.TUNE_CASE_ZOFF))  # Z-offset
        ...
        if self.pd.HAS_ZOFFSET_ITEM:
            ...
            self.lcd.Draw_Signed_Float(
                self.lcd.font8x16, self.lcd.Color_Bg_Black, 2, 2, 202, self.MBASE(
                    self.TUNE_CASE_ZOFF), self.pd.BABY_Z_VAR * 100
            )
    ...
    def Draw_Temperature_Menu(self):
        ...
        if self.pd.HAS_HOTEND:
            # Nozzle...
            self.lcd.Frame_AreaCopy(
                1, 197, 104, 238, 114, self.LBLX, self.MBASE(self.TEMP_CASE_TEMP))
            self.lcd.Frame_AreaCopy(
                1, 1, 89, 83, 101, self.LBLX + 44, self.MBASE(self.TEMP_CASE_TEMP))  # ...Temperature
        if self.pd.HAS_HEATED_BED:
            # Bed...
            self.lcd.Frame_AreaCopy(
                1, 240, 104, 264, 114, self.LBLX, self.MBASE(self.TEMP_CASE_BED))
            self.lcd.Frame_AreaCopy(
                1, 1, 89, 83, 101, self.LBLX + 27, self.MBASE(self.TEMP_CASE_BED))  # ...Temperature
        if self.pd.HAS_FAN:
            self.lcd.Frame_AreaCopy(1, 0, 119, 64, 132, self.LBLX, self.MBASE(
                self.TEMP_CASE_FAN))  # Fan speed
        if self.pd.HAS_HOTEND:
            # Preheat...
            self.lcd.Frame_AreaCopy(
                1, 107, 76, 156, 86, self.LBLX, self.MBASE(self.TEMP_CASE_PLA))
            self.lcd.Frame_AreaCopy(
                1, 157, 76, 181, 86, self.LBLX + 52, self.MBASE(self.TEMP_CASE_PLA))  # ...PLA
            self.lcd.Frame_AreaCopy(
                1, 131, 119, 182, 132, self.LBLX + 79, self.MBASE(self.TEMP_CASE_PLA))  # PLA setting
            # Preheat...
            self.lcd.Frame_AreaCopy(
                1, 107, 76, 156, 86, self.LBLX, self.MBASE(self.TEMP_CASE_ABS))
            self.lcd.Frame_AreaCopy(
                1, 172, 76, 198, 86, self.LBLX + 52, self.MBASE(self.TEMP_CASE_ABS))  # ...ABS
            self.lcd.Frame_AreaCopy(
                1, 131, 119, 182, 132, self.LBLX + 81, self.MBASE(self.TEMP_CASE_ABS))  # ABS setting
        ...
        if self.pd.HAS_HOTEND:
            i += 1
            self.Draw_Menu_Line(self.ICON_SetEndTemp +
                                (self.TEMP_CASE_TEMP) - 1)
            self.lcd.Draw_IntValue(
                True, True, 0, self.lcd.font8x16, self.lcd.Color_White, self.lcd.Color_Bg_Black,
                3, 216, self.MBASE(i),
                self.pd.thermalManager['temp_hotend'][0]['target']
        ...
        if self.pd.HAS_HOTEND:
            ...
            i += 1
            self.Draw_Menu_Line(self.ICON_SetEndTemp +
                                (self.TEMP_CASE_PLA) - 1)
            self.Draw_More_Icon(i)
            i += 1
            self.Draw_Menu_Line(self.ICON_SetEndTemp +
                                (self.TEMP_CASE_ABS) - 1)
            self.Draw_More_Icon(i)
    ...
    def Draw_Move_Menu(self):
        ...
        if self.pd.HAS_HOTEND:
            self.lcd.Frame_AreaCopy(
                1, 123, 192, 176, 202, self.LBLX, self.MBASE(4))  # "Extruder"
    ...
    def Goto_PrintProcess(self):
        ...
        if name:
            ...
            self.lcd.Draw_String(False, False, self.lcd.font8x16,
                                 self.lcd.Color_White, self.lcd.Color_Bg_Black, npos, 60, name)
    ...
    def Clear_Title_Bar(self):
        self.lcd.Draw_Rectangle(1, self.lcd.Color_Bg_Blue,
                                0, 0, self.lcd.DWIN_WIDTH, 30)

    def Clear_Menu_Area(self):
        self.lcd.Draw_Rectangle(
            1, self.lcd.Color_Bg_Black, 0, 31, self.lcd.DWIN_WIDTH, self.STATUS_Y)
    ...
    def Clear_Popup_Area(self):
        self.Clear_Title_Bar()
        self.lcd.Draw_Rectangle(
            1, self.lcd.Color_Bg_Black, 0, 31, self.lcd.DWIN_WIDTH, self.lcd.DWIN_HEIGHT)
    ...
    def Erase_Menu_Cursor(self, line):
        self.lcd.Draw_Rectangle(1, self.lcd.Color_Bg_Black, 0, self.MBASE(
            line) - 18, 14, self.MBASE(line + 1) - 20)

    def Erase_Menu_Text(self, line):
        self.lcd.Draw_Rectangle(1, self.lcd.Color_Bg_Black, self.LBLX, self.MBASE(
            line) - 14, 271, self.MBASE(line) + 28)
    ...
    def Add_Menu_Line(self):
        ...
        self.lcd.Draw_Line(self.lcd.Line_Color, 16, self.MBASE(
            self.MROWS + 1) - 20, 256, self.MBASE(self.MROWS + 1) - 19)

    def Scroll_Menu(self, dir):
        self.lcd.Frame_AreaMove(
            1, dir, self.MLINE, self.lcd.Color_Bg_Black, 0, 31, self.lcd.DWIN_WIDTH, 349)
    ...
    def Redraw_SD_List(self):
        ...
        if ed > 0:
            ...
            self.lcd.Draw_Rectangle(1, self.lcd.Color_Bg_Red, 10, self.MBASE(
                3) - 10, self.lcd.DWIN_WIDTH - 10, self.MBASE(4))
            self.lcd.Draw_String(False, False, self.lcd.font16x32, self.lcd.Color_Yellow, self.lcd.Color_Bg_Red, ((
                self.lcd.DWIN_WIDTH) - 8 * 16) / 2, self.MBASE(3), "No Media")
    ...
    def say_x(self, inset, line):
        self.lcd.Frame_AreaCopy(1, 95, 104, 102, 114,
                                self.LBLX + inset, line)  # "X"

    def say_y(self, inset, line):
        self.lcd.Frame_AreaCopy(1, 104, 104, 110, 114,
                                self.LBLX + inset, line)  # "Y"

    def say_z(self, inset, line):
        self.lcd.Frame_AreaCopy(1, 112, 104, 120, 114,
                                self.LBLX + inset, line)  # "Z"

    def say_e(self, inset, line):
        self.lcd.Frame_AreaCopy(1, 237, 119, 244, 129,
                                self.LBLX + inset, line)  # "E"
    ...
    def ICON_Prepare(self):
        if self.select_page.now == 1:
            ...
            self.lcd.Draw_Rectangle(
                0, self.lcd.Color_White, 145, 130, 254, 229)
    ...
    def ICON_Leveling(self, show):
        if show:
            ...
            self.lcd.Draw_Rectangle(
                0, self.lcd.Color_White, 145, 246, 254, 345)
    ...
    def ICON_Stop(self):
        if (self.select_print.now == 2):
            ...
            self.lcd.Draw_Rectangle(
                0, self.lcd.Color_White, 184, 252, 263, 351)
    ...
    def Item_Prepare_Disable(self, row):
        self.lcd.Frame_AreaCopy(1, 103, 59, 200, 74,
                                self.LBLX, self.MBASE(row))  # Disable Stepper"
        self.Draw_Menu_Line(row, self.ICON_CloseMotor)

    def Item_Prepare_Home(self, row):
        self.lcd.Frame_AreaCopy(1, 202, 61, 271, 71,
                                self.LBLX, self.MBASE(row))  # Auto Home"
        self.Draw_Menu_Line(row, self.ICON_Homing)

    def Item_Prepare_Offset(self, row):
        if self.pd.HAS_BED_PROBE:
            self.lcd.Frame_AreaCopy(
                1, 93, 179, 141, 189, self.LBLX, self.MBASE(row))  # "Z-Offset"
            self.lcd.Draw_Signed_Float(self.lcd.font8x16, self.lcd.Color_Bg_Black, 2, 2, 202, self.MBASE(
                row), self.pd.BABY_Z_VAR * 100)
        else:
            self.lcd.Frame_AreaCopy(
                1, 1, 76, 106, 86, self.LBLX, self.MBASE(row))  # "..."
        self.Draw_Menu_Line(row, self.ICON_SetHome)

    def Item_Prepare_PLA(self, row):
        self.lcd.Frame_AreaCopy(1, 107, 76, 156, 86,
                                self.LBLX, self.MBASE(row))  # Preheat"
        self.lcd.Frame_AreaCopy(1, 157, 76, 181, 86,
                                self.LBLX + 52, self.MBASE(row))  # PLA"
        self.Draw_Menu_Line(row, self.ICON_PLAPreheat)

    def Item_Prepare_ABS(self, row):
        self.lcd.Frame_AreaCopy(1, 107, 76, 156, 86,
                                self.LBLX, self.MBASE(row))  # "Preheat"
        self.lcd.Frame_AreaCopy(1, 172, 76, 198, 86,
                                self.LBLX + 52, self.MBASE(row))  # "ABS"
        self.Draw_Menu_Line(row, self.ICON_ABSPreheat)

    def Item_Prepare_Cool(self, row):
        self.lcd.Frame_AreaCopy(1, 200, 76, 264, 86,
                                self.LBLX, self.MBASE(row))  # "Cooldown"
    ...
    def EachMomentUpdate(self):
        ...
        if (self.checkkey == self.PrintProcess):
            if (self.pd.HMI_flag.print_finish and not self.pd.HMI_flag.done_confirm_flag):
                ...
                self.lcd.Draw_Rectangle(
                    1, self.lcd.Color_Bg_Black, 0, 250, self.lcd.DWIN_WIDTH - 1, self.STATUS_Y)
